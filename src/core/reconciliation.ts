 ...
 *
 * Strategy:
 * - Tasks identified by ID (unique identifier) to allow merging non-overlapping additions
...
export function reconcile(local: StoredData, server: StoredData, baseline: StoredData | null): MergeResult {
  ...
  // PHASE 1: Merge tasks by id
  const baseTasksById = new Map(base.tasks.map(t => [t.id, t]))
  const localTasksById = new Map(local.tasks.map(t => [t.id, t]))
  const serverTasksById = new Map(server.tasks.map(t => [t.id, t]))

  const allTaskIds = new Set([
    ...baseTasksById.keys(),
    ...localTasksById.keys(),
    ...serverTasksById.keys()
  ])

  const mergedTasksById = new Map<string, TaskData>()

  for (const taskId of allTaskIds) {
    const baseTask = baseTasksById.get(taskId)
    const localTask = localTasksById.get(taskId)
    const serverTask = serverTasksById.get(taskId)
    ...
    if (!baseExists && localExists && !serverExists) {
      log.debug(`Task added locally: "${localTask!.name}"`)
      mergedTasksById.set(taskId, localTask!)
    } else if (!baseExists && !localExists && serverExists) {
      log.debug(`Task added on server: "${serverTask!.name}"`)
      mergedTasksById.set(taskId, serverTask!)
    } else if (!baseExists && localExists && serverExists) {
      if (JSON.stringify(localTask) === JSON.stringify(serverTask)) {
        log.debug(`Task added identically on both sides: "${localTask!.name}"`)
        mergedTasksById.set(taskId, localTask!)
      } else {
        log.warn(`CONFLICT: Task created differently on both sides: "${localTask!.name}"`)
        conflicts.push({
          type: 'task-modified',
      ...
      if (JSON.stringify(localTask) === JSON.stringify(serverTask)) {
        log.debug(`Task unchanged on both sides: "${localTask!.name}"`)
        mergedTasksById.set(taskId, localTask!)
      } else if (JSON.stringify(baseTask) === JSON.stringify(serverTask)) {
        log.debug(`Task changed locally, server unchanged: "${localTask!.name}"`)
        mergedTasksById.set(taskId, localTask!)
      } else if (JSON.stringify(baseTask) === JSON.stringify(localTask)) {
        log.debug(`Task changed on server, local unchanged: "${serverTask!.name}"`)
        mergedTasksById.set(taskId, serverTask!)
      } else {
        log.warn(`CONFLICT: Task modified differently on both sides: "${baseTask!.name}"`)
        conflicts.push({
          type: 'task-modified',
      ...
      log.debug(`Task deleted locally, keeping deletion: "${baseTask!.name}"`)
    } else if (baseExists && localExists && !serverExists) {
      log.debug(`Task deleted on server, keeping deletion: "${baseTask!.name}"`)
    } else if (baseExists && !localExists && !serverExists) {
      log.debug(`Task deleted on both sides: "${baseTask!.name}"`)
    }
  }
  ...
  merged.tasks = Array.from(mergedTasksById.values())
